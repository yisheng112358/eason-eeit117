...
public class VerifyAioCheckOut extends PaymentVerifyBase {
	...
	public void verifyInvoice(InvoiceObj obj) {
		...
		// e. 統一編號[CustomerIdentifier]有值時 => CarruerType != 1 or 2, *Donation = 2, print
		// = 1
		...
		// [CarruerType]為'' or 1 時 => CarruerNum = '', [CarruerType]為 2， CarruerNum =
		// 固定長度為 16 且格式為 2 碼大小寫字母加上 14 碼數字。 [CarruerType]為 3 ，帶固定長度為 8 且格式為 1 碼斜線「/」加上由
		// 7 碼數字及大小寫字母組成
		...
		// 商品名稱含有管線 => 認為是多樣商品 *InvoiceItemName， *InvoiceItemCount ，*InvoiceItemWord，
		// *InvoiceItemPrice InvoiceItemTaxType逐一用管線分割，計算數量後與第一個比對
		if (obj.getInvoiceItemName().contains("|")) {
			...
			if (invCount.find()) {
				...
				if (itemCount != paramCount)
					throw new EcpayException("Count of item info InvoiceItemCount(" + paramCount
							+ ") not match item count from InvoiceItemName(" + itemCount + ")");
			}
			if (invWord.find()) {
				...
				if (itemCount != paramCount)
					throw new EcpayException("Count of item info InvoiceItemWord(" + paramCount
							+ ") not match item count from InvoiceItemName(" + itemCount + ")");
			}
			if (invPrice.find()) {
				...
				if (itemCount != paramCount)
					throw new EcpayException("Count of item info InvoiceItemPrice(" + paramCount
							+ ") not match item count from InvoiceItemName(" + itemCount + ")");
			}
			if (invType.find()) {
				...
				if (itemCount != paramCount)
					throw new EcpayException("Count of item info InvoiceItemTaxType(" + paramCount
							+ ") not match item count from InvoiceItemName(" + itemCount + ")");
			}
			// 課稅類別[TaxType] = 9 時 => InvoiceItemTaxType 能含有1,2 3(and at least contains one
			// 1 and other)
